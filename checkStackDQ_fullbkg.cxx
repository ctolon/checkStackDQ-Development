...
// This macro usage for Full Background
...
int checkStackDQ_fullbkg()
{
	...
	std::cout << "                           Usage of this macro for full background events"                    << '\n';
	std::cout << "============================================================================================" << '\n';
	...
	std::cout << "============================================================================================" << '\n';
	std::cin >> kAntiSelectionPdg;
	//if(kAntiSelectionPdg != 13 || kAntiSelectionPdg != 11){printf("Your LeptonPDG selection is wrong!");}
	assert(kAntiSelectionPdg == 13 || kAntiSelectionPdg == 11);

	// Settings for Selections.
	//if(kAntiSelectionPdg == 11) { kYRangeMin = -1.5; kYRangeMax = 1.5;  kMidYSimulation     = kTRUE; }
	//if(kAntiSelectionPdg == 13) { kYRangeMin = -4.3; kYRangeMax = -2.3; kForwardYSimulation = kTRUE; }


	// set eta, phi and theta ranges
	kYRangeMin = -4; kYRangeMax = 4;
  ...
  /*
	...
	*/
	...
	Float_t kPx, kPy;
	Float_t kVx, kVy, kVz;
	Float_t kPt, kY, kEta, kPhi, kTheta;
	...
	////////////////////////////////
	/// Histograms For Branching ////
	/////////////////////////////////

	/// define histos for other background particles before cuts

	TH1F *histPtPairOtherParticles   = new TH1F("PtPairUnknownPromptBeforeCuts",";p_{T}(GeV/c);Entries",100,0.,20.);
	TH1F *histMassPairOtherParticles = new TH1F("MassUnknownPromptBeforeCuts",";m_{ll}(GeV/c^{2});Entries",5./0.02,0.,5.);
	TH1F *histYOtherParticles        = new TH1F("YUnknownPromptBeforeCuts",";y;Entries",100,kYRangeMin,kYRangeMax);
	TH1F *histPtOtherParticles       = new TH1F("PtUnknownPromptBeforeCuts",";p_{T}(GeV/c);Entries",100,0.,15.);
	TH1F *histEtaOtherParticles      = new TH1F("EtaUnknownPromptBeforeCuts",";#eta;Entries",100,kEtaRangeMin,kEtaRangeMax);
	TH1F *histThetaOtherParticles    = new TH1F("ThetaUnknownPromptBeforeCuts",";#theta (rad.);Entries",100,kThetaRangeMin,kThetaRangeMax);
	TH1F *histPhiOtherParticles      = new TH1F("PhiUnknownPromptBeforeCuts",";#phi (rad.);Entries",100,kPhiRangeMin,kPhiRangeMax);

	/// define histos for other background particles after cuts

	TH1F *histPtPairOtherParticlesAfterCuts   = new TH1F("PtPairUnknownAfterCuts",";p_{T}(GeV/c);Entries",100,0.,20.);
	TH1F *histMassPairOtherParticlesAfterCuts = new TH1F("MassUnknownAfterCuts",";m_{ll}(GeV/c^{2});Entries",5./0.02,0.,5.);
	TH1F *histYOtherParticlesAfterCuts        = new TH1F("YUnknownAfterCuts",";y;Entries",100,kYRangeMin,kYRangeMax);
	TH1F *histPtOtherParticlesAfterCuts       = new TH1F("PtUnknownAfterCuts",";p_{T}(GeV/c);Entries",100,0.,15.);
	TH1F *histEtaOtherParticlesAfterCuts      = new TH1F("EtaUnknownAfterCuts",";#eta;Entries",100,kEtaRangeMin,kEtaRangeMax);
	TH1F *histThetaOtherParticlesAfterCuts    = new TH1F("ThetaUnknownPromptAfterCuts",";#theta (rad.);Entries",100,kThetaRangeMin,kThetaRangeMax);
	TH1F *histPhiOtherParticlesAfterCuts      = new TH1F("PhiUnknownPromptAfterCuts",";#phi (rad.);Entries",100,kPhiRangeMin,kPhiRangeMax);
	...
	histPtPairOtherParticles->Sumw2();
	histPtOtherParticles->Sumw2();
	histPtPairOtherParticlesAfterCuts->Sumw2();
	histPtOtherParticlesAfterCuts->Sumw2();
  ...
  NonPromptJpsiTree->Branch("kPt", &kPt, "kPt/F");
	...
	/// Define particles without j/psi + psi(2S)
	TTree *otherParticlesTree = new TTree("otherParticlesTree", "TTree with a structure before cuts");
	otherParticlesTree->Branch("kInvMassPair", &kInvMassPair, "kInvMassPair/F");
	otherParticlesTree->Branch("kPt", &kPt, "kPt/F");
	otherParticlesTree->Branch("kPtPair", &kPtPair, "kPtPair/F");
	otherParticlesTree->Branch("kY", &kY, "kY/F");
	otherParticlesTree->Branch("kEta", &kEta, "kEta/F");
	otherParticlesTree->Branch("kTheta", &kTheta, "kTheta/F");
	otherParticlesTree->Branch("kPhi", &kPhi, "kPhi/F");

	TTree *otherParticlesTreeAfterCuts = new TTree("otherParticlesTreeAfterCuts", "TTree with a structure before cuts");
	otherParticlesTreeAfterCuts->Branch("kInvMassPair", &kInvMassPairBranchCut, "kInvMassPair/F");
	otherParticlesTreeAfterCuts->Branch("kPt", &kPtBranchCut, "kPt/F");
	otherParticlesTreeAfterCuts->Branch("kPtPair", &kPtPairBranchCut, "kPtPair/F");
	otherParticlesTreeAfterCuts->Branch("kY", &kYBranchCut, "kY/F");
	otherParticlesTreeAfterCuts->Branch("kEta", &kEtaBranchCut, "kEta/F");
	otherParticlesTreeAfterCuts->Branch("kTheta", &kThetaBranchCut, "kTheta/F");
	otherParticlesTreeAfterCuts->Branch("kPhi", &kPhiBranchCut, "kPhi/F");

	/// For MachineLearning, Branches for all particles
	TTree *allCandidatesTree = new TTree("allCandidatesTree", "TTree with a structure before cuts");
	allCandidatesTree->Branch("kInvMassPair", &kInvMassPair, "kInvMassPair/F");
	allCandidatesTree->Branch("kPt", &kPt, "kPt/F");
	allCandidatesTree->Branch("kPtPair", &kPtPair, "kPtPair/F");
	allCandidatesTree->Branch("kY", &kY, "kY/F");
	allCandidatesTree->Branch("kEta", &kEta, "kEta/F");
	allCandidatesTree->Branch("kTheta", &kTheta, "kTheta/F");
	allCandidatesTree->Branch("kPhi", &kPhi, "kPhi/F");

	TTree *allCandidatesTreeAfterCuts = new TTree("allCandidatesTreeAfterCuts", "TTree with a structure before cuts");
	allCandidatesTreeAfterCuts->Branch("kInvMassPair", &kInvMassPairBranchCut, "kInvMassPair/F");
	allCandidatesTreeAfterCuts->Branch("kPt", &kPtBranchCut, "kPt/F");
	allCandidatesTreeAfterCuts->Branch("kPtPair", &kPtPairBranchCut, "kPtPair/F");
	allCandidatesTreeAfterCuts->Branch("kY", &kYBranchCut, "kY/F");
	allCandidatesTreeAfterCuts->Branch("kEta", &kEtaBranchCut, "kEta/F");
	allCandidatesTreeAfterCuts->Branch("kTheta", &kThetaBranchCut, "kTheta/F");
	allCandidatesTreeAfterCuts->Branch("kPhi", &kPhiBranchCut, "kPhi/F");

	//TODO : Set range of branches for matching in MachineLearning

	/// J/Psi and Psi(2S) Pdg Codes (Mother Particles)
	...
	for (int eventID = 0; eventID < mcbr->GetEntries(); ++eventID) {
		...
		for (auto& t : *mctracks) {
			...
			Bool_t isPrompt = kMotherTrackID <= 0 && (isJpsi == kTRUE || isPsi2S == kTRUE)  ? kTRUE : kFALSE;
			// DEBUG
			...
			if(!isPrompt) { // if is not prompt, check beauty mother
			...
			Bool_t isPhoton = kFirstDaughterTrackID == -1 && kLastDaughterTrackID == -1;
			// Check is photon?
			if(isPhoton) continue;
			//LOG(DEBUG) << " mother track - pdg: " << t.GetPdgCode() << "\n" << " first daughter: "<<  kFirstDaughterTrackID << "\n" << " last daughter: " << kLastDaughterTrackID << "\n";

			for(int idaugh=kFirstDaughterTrackID; idaugh<kLastDaughterTrackID+1; idaugh++ ) {
				auto td = mcreader.getTrack(eventID, idaugh);
				//std::cout << "PDG Code in Loop:" << td->GetPdgCode() << '\n';
				//std::cout << "-----------------------------------------------" << '\n';
				//kPair     = (o2::MCTrack*)td; kAntiPair = (o2::MCTrack*)td;
				// Lepton Track Selection Part (e- e+ and mu+ mu-)
				//if(td->GetPdgCode() == 11) { kPair     = (o2::MCTrack*)td; kAntiSelectionPdg = 11;  kMidYSimulation = kTRUE; dilectronCounter++; }
				//if(td->GetPdgCode() == 13) { kPair     = (o2::MCTrack*)td; kAntiSelectionPdg = 13;  kForwardYSimulation = kTRUE; dimuonCounter++;  }
				//if(td->GetPdgCode() == -11){ kAntiPair = (o2::MCTrack*)td; kAntiSelectionPdg = 11; kMidYSimulation = kTRUE; }
				//if(td->GetPdgCode() == -13){ kAntiPair = (o2::MCTrack*)td; kAntiSelectionPdg = 13; kForwardYSimulation = kTRUE; }
				if(td->GetPdgCode() > 0) { kPair = (o2::MCTrack*)td; kAntiSelectionPdg = td->GetPdgCode(); }
				if(td->GetPdgCode() < 0) { kAntiPair = (o2::MCTrack*)td; kAntiSelectionPdg = td->GetPdgCode(); }
			}
			// DEBUG cout
			//std::cout << "Lepton Adress: " << kPair << '\n';
			//std::cout << "AntiLepton Adress: " << kAntiPair << '\n';

			// TODO: Needed semihadronic selections.
			...
			// TODO: Bkg için other particlestan gelenler için istatistik çıkar.
			...
			//LOG(DEBUG) << "antiLepton mass Value: " << TDatabasePDG::Instance()->GetParticle(antileptonPDG)->Mass() << "\n";
			...
			//////////////////////////////////////////////////////////////////////////////
			// For Machine Learning Selection List
			//if(kInvMassPair > 3.2 || kInvMassPair < 3.0) continue;
		  //if(kInvMassPair > 3.3 || kInvMassPair < 2.9) continue;
			//if(kInvMassPair > 3.4 && !isJpsi) continue;
			// Mass Selection for J/Psi
			//if(!isJpsi) continue;
			...
			//if(kInvMassPair < continue;
			//if(kInvMassPair > 1.2 || kInvMassPair < 0.8) continue;
			///////////////////////////////////////////////////////////////////////////////////
			...
			*/
			...
			//// fill other background particles
			else{
				histPtPairOtherParticles->Fill(kPtPair);
				histMassPairOtherParticles->Fill(kInvMassPair);
				histPtOtherParticles->Fill(kPt);
				histYOtherParticles->Fill(kY);
				histEtaOtherParticles->Fill(kEta);
				histThetaOtherParticles->Fill(kTheta);
				histPhiOtherParticles->Fill(kPhi);
				histOrigin->Fill(4.5);
				otherParticlesTree->Fill();
			}
			// Fill all candiates
			...
			*/
			...
			kMassPair1 = TDatabasePDG::Instance()->GetParticle(kAntiSelectionPdg)->Mass();
			kMassPair2 = TDatabasePDG::Instance()->GetParticle(kAntiSelectionPdg)->Mass();
			kInvMassPair =  kMassPair1*kMassPair1+kMassPair2*kMassPair2 + 2.0*(TMath::Sqrt(kMassPair1*kMassPair1+kPair->GetP()*kPair->GetP())*TMath::Sqrt(kMassPair2*kMassPair2+kAntiPair->GetP()*kAntiPair->GetP()) - kPair->Px()*kAntiPair->Px() - kPair->Py()*kAntiPair->Py() - kPair->Pz()*kAntiPair->Pz());
			kInvMassPair = TMath::Sqrt(kInvMassPair);
			////
			kPx = kPair->Px()+kAntiPair->Px();
      ...
      /*
			...
			*/
			...
			//// fill other background particles
				...
				histPtPairOtherParticlesAfterCuts->Fill(kPtPair);
				histMassPairOtherParticlesAfterCuts->Fill(kInvMassPair);
				histPtOtherParticlesAfterCuts->Fill(kPt);
				histYOtherParticlesAfterCuts->Fill(kY);
				histEtaOtherParticlesAfterCuts->Fill(kEta);
				histThetaOtherParticlesAfterCuts->Fill(kTheta);
				histPhiOtherParticlesAfterCuts->Fill(kPhi);
				histOriginAfterCuts->Fill(4.5);
				otherParticlesTreeAfterCuts->Fill();
			}
			// Fill All Candiates
			allCandidatesTreeAfterCuts->Fill();
	...
	*/
	...
	TFile foutput1("histBkgKine.root","RECREATE");
	allCandidatesTree->Write();
	otherParticlesTree->Write();
	...
	histPtPairOtherParticles->Write();
	histMassPairOtherParticles->Write();
	histPtOtherParticles->Write();
	histYOtherParticles->Write();
	histEtaOtherParticles->Write();
	histThetaOtherParticles->Write();
	histPhiOtherParticles->Write();


	/// Writing Histograms to Disk (Output will histCutKine.root file) --> For After Cuts
	TFile foutput2("histBkgCutKine.root","RECREATE");
	allCandidatesTreeAfterCuts->Write();
	otherParticlesTreeAfterCuts->Write();
	...
	TFile foutput3("histBkgCompareAfterCuts.root","RECREATE");
 ...
 */
	...
	TFile foutput4("histBkgFittingAfterCuts.root","RECREATE");
	...
	TFile foutput5("histBkgAcceptance.root","RECREATE");
	...
	TFile foutput6("histBkgAnalysis.root","RECREATE");
	...
	TFile foutput7("histBkgCutAnalysis.root","RECREATE");
	...
	TFile foutput8("histBkgDetectorAnalysis.root","RECREATE");
	...
	TFile foutput9("histBkgBeautyAnalysis.root","RECREATE");
	...
	TFile foutput10("histBkgStatistics.root","RECREATE");
	...
	TFile foutput11("histBkgCutStatistics.root","RECREATE");
	histOriginAfterCuts->Write();
