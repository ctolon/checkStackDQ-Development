...
int checkStackDQ_sgn(Bool_t isMidYSimulation = kTRUE, Bool_t createTTree = kTRUE)
{
  // Define Rapidity, Lepton PDG and Sim Rapidity Type
  Int_t kSelectionPdg;
  Double_t kYRangeMin, kYRangeMax;
  // Bool_t isMidYSimulation = kFALSE;
  ...
  TFile foutput2("TTreesSgnKine.root", "RECREATE");
  ...
  if (isMidYSimulation == kTRUE) {
    kYRangeMin = -1.5;
    kYRangeMax = 1.5;
    kSelectionPdg = 11;
  }
  if (isMidYSimulation == kFALSE) {
    kYRangeMin = -4.3;
    kYRangeMax = -2.3;
    kSelectionPdg = 13;
  }
  ...
  /// Defination All Variables we need
  ...
  /// Variables for Reading Kinematics After Cuts
  ...
  Int_t kCandidateJpsiPdg = kMotherPdgCode[0];
  Int_t kCandidatePsi2SPdg = kMotherPdgCode[1];

  Double_t kCandidateJpsiMass = TDatabasePDG::Instance()->GetParticle(kCandidateJpsiPdg)->Mass();
  Double_t kCandidatePsi2SMass = TDatabasePDG::Instance()->GetParticle(kCandidatePsi2SPdg)->Mass();
  ...
  // TODO: Add gamma stats for prompt and nonprompt (not prompt + prompt, It's already done)
  ...
  for (int eventID = 0; eventID < getBranchMCTrack->GetEntries(); ++eventID) {
    ...
    for (auto& t : *mctracks) {
      ...
      // condition for fill histos
      Bool_t isJpsi = TMath::Abs(t.GetPdgCode()) == kCandidateJpsiPdg;
      Bool_t isPsi2S = TMath::Abs(t.GetPdgCode()) == kCandidatePsi2SPdg;
      for (Int_t candidate : kMotherPdgCode) {
        if (TMath::Abs(t.GetPdgCode()) == candidate) {
          ...
          o2::MCTrack* kGammaFromPair = 0x0;     // just for checking
          o2::MCTrack* kGammaFromAntiPair = 0x0; // just for checking
          ...
          */
          ...
          //auto tdmdebug = mcreader.getTrack(eventID, kSecondMotherTrackID);
          //LOG(debug) << "PDG (Second Beauty Mother Track): " << TMath::Abs(tdmdebug->GetPdgCode());
          ...
          if (!isPrompt) { //  check beauty mother
            ...
            LOG(debug) << "PDG (Beauty Mother Track): " << TMath::Abs(tdM->GetPdgCode());

            // get second mothertrack
            //auto tdSecondM = mcreader.getTrack(eventID, kSecondMotherTrackID);
            //LOG(debug) << "PDG (Second Mother Track): " << TMath::Abs(tdSecondM->GetPdgCode());
            ...
            for (int i = 0; i < kSizeBeautyPdgCode; i++) {
              if (TMath::Abs(tdM->GetPdgCode()) == kBeautyPdgCode[i]) {
                ...
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateBzeroPdg)
                  kCandidateBzero = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateBplusPdg)
                  kCandidateBplus = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateBszeroPdg)
                  kCandidateBszero = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateBcplusPdg)
                  kCandidateBcplus = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateSigmaBPdg)
                  kCandidateSigmaB = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateLambdaBzeroPdg)
                  kCandidateLambdaBzero = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateXiBminusPdg)
                  kCandidateXiBminus = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateXiBzeroPdg)
                  kCandidateXiBzero = (o2::MCTrack*)tdM;
                if (TMath::Abs(tdM->GetPdgCode()) == kCandidateOmegaBminusPdg)
          ...
          for (int idaugh = kFirstDaughterTrackID; idaugh < kLastDaughterTrackID + 1; idaugh++) {
            ...
            if (kDaughterTrack->GetPdgCode() == kSelectionPdg && kDaughterTrack->GetPdgCode() == 22)
              //kGammaFromPair = (o2::MCTrack*)kSecondaryDaughterTrack;
              kGammaFromPair = (o2::MCTrack*)kDaughterTrack;
            if (kDaughterTrack->GetPdgCode() == kSelectionPdg * -1 && kDaughterTrack->GetPdgCode() == 22)
              //kGammaFromAntiPair = (o2::MCTrack*)kSecondaryDaughterTrack;
              kGammaFromAntiPair = (o2::MCTrack*)kDaughterTrack;
          }

          // Selection Cuts for dilepton pairs
          if ((!kPair) || (!kAntiPair))
            continue;
          ...
          */
          ...
          if (isPrompt && isJpsi) {
            ...
            if (kGamma != NULL)
              histRadiativeYJpsiPrompt->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairPsi2sPrompt->Fill(kRadiativePtPair);
            histMassPairPsi2sPrompt->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairPsi2sPrompt->Fill(kRadiativeInvMassPair);
            histPtPsi2sPrompt->Fill(kPt);
            histYPsi2sPrompt->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYPsi2sPrompt->Fill(kY);
          ...
          /// fill non prompt
            ...
            if (kGamma != NULL)
              histRadiativePtPairJpsiNonPrompt->Fill(kRadiativePtPair);
            histMassPairJpsiNonPrompt->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairJpsiNonPrompt->Fill(kRadiativeInvMassPair);
            histPtJpsiNonPrompt->Fill(kPt);
            histYJpsiNonPrompt->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYJpsiNonPrompt->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairPsi2sNonPrompt->Fill(kRadiativePtPair);
            histMassPairPsi2sNonPrompt->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairPsi2sNonPrompt->Fill(kRadiativeInvMassPair);
            histPtPsi2sNonPrompt->Fill(kPt);
            histYPsi2sNonPrompt->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYPsi2sNonPrompt->Fill(kY);
          ...
          kRadiativeInvMassPairBranchCut = kRadiativeInvMassPair;
          ...
          if (isPrompt && isJpsi) {
            ...
            if (kGamma != NULL)
              histRadiativePtPairJpsiPromptAfterCuts->Fill(kRadiativePtPair);
            histMassPairJpsiPromptAfterCuts->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairJpsiPromptAfterCuts->Fill(kRadiativeInvMassPair);
            histPtJpsiPromptAfterCuts->Fill(kPt);
            histYJpsiPromptAfterCuts->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYJpsiPromptAfterCuts->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairEfficiencyJpsiPrompt->Fill(kRadiativePtPair);
            if (kGamma != NULL)
              histRadiativeYEfficiencyJpsiPrompt->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairPsi2sPromptAfterCuts->Fill(kRadiativePtPair);
            histMassPairPsi2sPromptAfterCuts->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairPsi2sPromptAfterCuts->Fill(kRadiativeInvMassPair);
            histPtPsi2sPromptAfterCuts->Fill(kPt);
            histYPsi2sPromptAfterCuts->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYPsi2sPromptAfterCuts->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairEfficiencyPsi2sPrompt->Fill(kRadiativePtPair);
            if (kGamma != NULL)
              histRadiativeYEfficiencyPsi2sPrompt->Fill(kY);
          ...
          /// fill non prompt
            ...
            if (kGamma != NULL)
              histRadiativePtPairJpsiNonPromptAfterCuts->Fill(kRadiativePtPair);
            histMassPairJpsiNonPromptAfterCuts->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairJpsiNonPromptAfterCuts->Fill(kRadiativeInvMassPair);
            histPtJpsiNonPromptAfterCuts->Fill(kPt);
            histYJpsiNonPromptAfterCuts->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYJpsiNonPromptAfterCuts->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairEfficiencyJpsiNonPrompt->Fill(kRadiativePtPair);
            if (kGamma != NULL)
              histRadiativeYEfficiencyJpsiNonPrompt->Fill(kY);
            ...
            if (kGamma != NULL)
              histRadiativePtPairPsi2sNonPromptAfterCuts->Fill(kPtPair);
            histMassPairPsi2sNonPromptAfterCuts->Fill(kInvMassPair);
            if (kGamma != NULL)
              histRadiativeMassPairPsi2sNonPromptAfterCuts->Fill(kRadiativeInvMassPair);
            histPtPsi2sNonPromptAfterCuts->Fill(kPt);
            histYPsi2sNonPromptAfterCuts->Fill(kY);
            if (kGamma != NULL)
              histRadiativeYPsi2sNonPromptAfterCuts->Fill(kY);
            ...
            histYEfficiencyPsi2sPrompt->Fill(kY);
            if (kGamma != NULL)
              histRadiativePtPairEfficiencyPsi2sNonPrompt->Fill(kRadiativePtPair);
            if (kGamma != NULL)
              histRadiativeYEfficiencyPsi2sNonPrompt->Fill(kY);
          ...
          delete kAntiPair;
          // Charmonium
          delete kCandidateJpsi;
          delete kCandidatePsi2S;
          // Beauties
